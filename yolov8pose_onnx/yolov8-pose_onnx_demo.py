...
def detect(img_path):
    ...
    ort_session = ort.InferenceSession('./yolov8pos_relu_zq.onnx')
    ...
    for i in range(len(predbox)):
        ...
        cv2.rectangle(orig, (xmin, ymin), (xmax, ymax), (128, 128, 0), 2)
        ptext = (xmin, ymin)
        ...
        for i in range(0, keypoint_num):
            if pose[i * 3] > 0.5:
                if i < 5:
                    ...
                    color = color_list[2]
                cv2.circle(orig, (int(pose[i * 3 + 1]), int(pose[i * 3 + 2])), 2, color, 5)
        ...
        for i, sk in enumerate(skeleton):
            if pose[(sk[0] - 1) * 3] > 0.5:
                ...
                if (sk[0] - 1) < 5:
                    ...
                    color = color_list[2]
                cv2.line(orig, pos1, pos2, color, thickness=2, lineType=cv2.LINE_AA)
